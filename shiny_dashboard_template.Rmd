...
rank = world_cup %>% group_by(country)%>% pull(rank)
# sliderInput widget for rank
sliderInput(
  "rank_range", 
  label = h3("Official 2022 FIFA Rank"), 
  min=0, max=61, value = c(3))
...
world_cup = 
  world_cup%>%
  mutate(results = -1.56 + 0.660*pld - 0.628*d + 0.0159*rank + 0.154*gf - 0.224*ga) 
...
renderPrint({
  world_cup %>%
    ...
    mutate( 
      results= -1.56*input$number_games_range[1])
...
In our dataset, for those that made it into the world cup, the  `r renderPrint({input[["c"]]})`
