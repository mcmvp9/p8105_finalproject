...
final_merge = 
  ...
  mutate(
    ...
    ## Add missing area and land area 
    area = case_when(country %in% c('country') ~ 'add value', TRUE ~ as.character(area)),
   

    ## Add missing land area
    land_area_km = case_when(country %in% c('country') ~ 'add value',
                             TRUE ~ as.character(land_area_km)),
...
```{r}
write_csv(final_merge, "worldcup_dataset.csv")
```
